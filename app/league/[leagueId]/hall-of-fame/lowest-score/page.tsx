...
import { loadHofLowestScoreHistory } from "@/lib/queries/hof-winning-score-history";
...
export default async function HofLowestScorePage({
  params,
  searchParams,
}: PageProps) {
  ...
  if (!user) {
    redirect(`/login?next=/league/${slug}/hall-of-fame/lowest-score`);
  }

  const data = await loadHofLowestScoreHistory({
    leagueSlug: slug,
    userId: user.id,
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6">
      <HofTitlePageHeader
        ...
        description="Lowest single-game scores across league history."
      />
