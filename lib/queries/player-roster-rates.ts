...
type RosterRatesEntries = Array<[string, PlayerRosterRates]>;

const getCachedRosterRatesEntries = createProcessCache<RosterRatesEntries>({
  ttlMs: 10 * 60 * 1000,
  maxEntries: 128,
});
...
function rosterRatesCacheKey(playerIds: string[]): string {
  return [...new Set(playerIds)].sort().join(",");
}

async function loadPlayerRosterRatesMap(
  playerIds: string[],
): Promise<Map<string, PlayerRosterRates>> {
...
export const getPlayerRosterRatesMap = cache(
  async (playerIds: string[]): Promise<Map<string, PlayerRosterRates>> => {
    ...
    const entries = await getCachedRosterRatesEntries(
      rosterRatesCacheKey(playerIds),
      async () => [...(await loadPlayerRosterRatesMap(playerIds)).entries()],
    );
